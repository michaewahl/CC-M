...
class TestCostCalculation:
    def test_haiku_pricing(self):
        ...
        cost = calculate_cost("claude-haiku-4-5-20241022", 1000, 1000)
        expected = (1000 * 1.0 + 1000 * 5.0) / 1_000_000
        assert abs(cost - expected) < 1e-9

    def test_sonnet_pricing(self):
        cost = calculate_cost("claude-sonnet-4-6-20250514", 1000, 1000)
        expected = (1000 * 3.0 + 1000 * 15.0) / 1_000_000
        assert abs(cost - expected) < 1e-9

    def test_opus_pricing(self):
        cost = calculate_cost("claude-opus-4-6-20250514", 1000, 1000)
    ...
    def test_unknown_model_uses_opus_pricing(self):
        ...
        opus_cost = calculate_cost("claude-opus-4-6-20250514", 1000, 1000)
        assert cost == opus_cost

    def test_zero_tokens(self):
        assert calculate_cost("claude-haiku-4-5-20241022", 0, 0) == 0.0

    def test_opus_is_most_expensive(self):
        haiku = calculate_cost("claude-haiku-4-5-20241022", 10000, 5000)
        sonnet = calculate_cost("claude-sonnet-4-6-20250514", 10000, 5000)
        opus = calculate_cost("claude-opus-4-6-20250514", 10000, 5000)
...
class TestCostTracker:
    ...
    def test_log_and_stats(self, tmp_path):
        ...
        tracker.log_request("claude-haiku-4-5-20241022", "SIMPLE", 1.0, 1000, 500)
        tracker.log_request("claude-sonnet-4-6-20250514", "MEDIUM", 3.0, 2000, 1000)
    ...
    def test_savings_are_positive_for_cheap_models(self, tmp_path):
        ...
        record = tracker.log_request("claude-haiku-4-5-20241022", "SIMPLE", 1.0, 10000, 5000)
    ...
    def test_savings_are_zero_for_opus(self, tmp_path):
        ...
        record = tracker.log_request("claude-opus-4-6-20250514", "COMPLEX", 5.0, 10000, 5000)
    ...
    def test_recent_requests_limited(self, tmp_path):
        tracker = self._make_tracker(tmp_path)
        for i in range(15):
            tracker.log_request("claude-haiku-4-5-20241022", "SIMPLE", 1.0, 100, 50)
    ...
    def test_savings_percent(self, tmp_path):
        tracker = self._make_tracker(tmp_path)
        tracker.log_request("claude-haiku-4-5-20241022", "SIMPLE", 1.0, 10000, 5000)
